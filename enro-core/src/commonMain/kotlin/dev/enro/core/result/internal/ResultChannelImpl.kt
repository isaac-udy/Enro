    ...
    internal fun consumeResult(pendingResult: PendingResult) {
        ...
        when (pendingResult) {
            ...
            is PendingResult.Result -> {
                ...
                val key = pendingResult.navigationKey
                if (!properties.resultType.isInstance(result)) {
                    throw EnroException.ReceivedIncorrectlyTypedResult("Attempted to consume result with wrong type; expended ${properties.resultType.simpleName} but was ${result::class.simpleName}")
                }
                result as Result
                key as Key
                properties.navigationHandle.runWhenHandleActive {
