    ...
    object Serializer : KSerializer<WrappedMap> {
        ...
        override fun serialize(encoder: Encoder, value: WrappedMap) {
            encoder.encodeStructure(descriptor) {
                encodeSerializableElement(
                    ...
                    value = value.value
                        .map { (key, value) ->
                            key.wrapForSerialization() to value.wrapForSerialization()
                        }
                        .toMap(),
                )
            }
        ...
        override fun deserialize(decoder: Decoder): WrappedMap {
            val list = decoder
                ...
                .map { (key, value) ->
                    key.unwrapForSerialization() to value.unwrapForSerialization()
                }
                .toMap()
