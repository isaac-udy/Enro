...
public object DefaultComposableExecutor :
    ...
    override fun close(context: NavigationContext<out ComposableDestination>) {
        val container = context.contextReference.parentContainer
        container.setBackstack(container.backstackFlow.value.close(context.contextReference.instruction.instructionId))
    }
}
...
private fun openComposableAsActivity(
    ...
    val fragmentInstruction = fromContext.controller.dependencyScope.get<NavigationHostFactory>()
        .createHostFor<Fragment>(instruction)

    val activityInstruction = fromContext.controller.dependencyScope.get<NavigationHostFactory>()
        .createHostFor<Activity>(fragmentInstruction)

    fromContext.controller.dependencyScope.get<ExecuteOpenInstruction>().invoke(
        fromContext,
        activityInstruction.asDirection(direction)
    )
}
