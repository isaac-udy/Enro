package dev.enro.compose.destination
...
internal class ComposableDestinationOwner(
    ...
    private val renderInEnroEnvironment: RenderComposableInEnroEnvironment,
    ...
    init {
        ...
        destination.parentContainer = parentContainer
        destination.lifecycleOwner = this
        destination.viewModelStoreOwner = this
        destination.savedStateRegistryOwner = this
        destination.hasDefaultViewModelProviderFactory = this
        destination.context = ComposeContext(destination)
    ...
    private fun ProvideRenderingEnvironment(
        ...
        CompositionLocalProvider(
            ...
            LocalNavigationHandle provides remember { viewModelStoreOwner.getNavigationHandle() }
        ) {
            saveableStateHolder.SaveableStateProvider(key = instruction.instructionId) {
                renderInEnroEnvironment {
                    content()
                }
            }
        }
